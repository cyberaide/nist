...
\cite{nist-bd}. From this document we gather that the {\it
  ...
  vertical data-based applications.''}  It will produce a number of documents
  related to definitions \cite{nist-bd-v1}, taxonomies \cite{nist-bd-v2}, use
  cases and general requirements \cite{nist-bd-v3}, security and privacy
  \cite{nist-bd-v4}, architectures white paper survey \cite{nist-bd-v5},
  reference architecture \cite{nist-bd}, standards roadmap \cite{nist-bd-v7},
  and an interface definition \cite{??}.
...
The following incantations may be used to create and deploy the stack
shown in Figure~\ref{F:stack-graph}. These commands will create the
\emph{fingerprint} stack from preexisting \emph{spark}, \emph{hbase},
and \emph{drill} stacks and compose it with the predefined
\emph{hadoop} and \emph{web} stacks. Then, a cluster of four nodes is
...
Evaluation of \emph{My Analytics Stack} requires the evaluation of the \emph{Fingerprint} and \emph{Web} stacks, each of which are compositions of other stacks.
